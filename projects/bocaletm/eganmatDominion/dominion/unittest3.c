...
int testAmbassador() {
  ...
  clearHand(0,gamePtr);
  failed = assertMod(ambassadorCard(1, -1, 0, 0, gamePtr),-1);
  if (failed) {
    printf("invalid choice2 allowed\n");
  }

  clearHand(0,gamePtr);
  failed = assertMod(ambassadorCard(1, 3, 0, 0, gamePtr),-1);
  if (failed) {
    printf("invalid choice2 allowed\n");
  }

  clearHand(0,gamePtr);
  failed = assertMod(ambassadorCard(1, 0, 1, 0, gamePtr),-1);
  if (failed) {
  ...
  ambassadorCard(0, 1, 1, 0, gamePtr);
  ...
  gainCard(mine, gamePtr, 2, 0);
  gamePtr->supplyCount[mine] = 0;
  ambassadorCard(1, 1, 0, 0, gamePtr);
  ...
  gamePtr->supplyCount[mine] = 0;
  failed = assertMod(ambassadorCard(1, 2, 0, 0, gamePtr),-1);
  if (failed) {
  ...
  gainCard(copper, gamePtr, 2, 0);
  gamePtr->supplyCount[mine] = 0;
  ambassadorCard(1, 2, 0, 0, gamePtr);
